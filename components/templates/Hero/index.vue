...
<template>
  <section id="hero"
    class="hero bg-primary-background text-white text-center relative overflow-hidden min-h-[500px]"
  >
    <ImageSlider class="inset-0 top-0 left-0"/>
    <div
      class="w-full flex flex-col items-center justify-center space-y-4 p-8 absolute top-0 left-0 h-full z-30 bg-[rgba(255,138,138,0.4)]"
    >
      ...
      <div class="flex justify-center space-x-8 text-3xl pt-3 uppercase" v-if="!isPastTargetTime">
        ...
        <div>
          <span class="block font-bold">{{ hours }}</span>
          <span class="text-base">Giờ</span>
        </div>
        <div>
          <span class="block font-bold">{{ minutes }}</span>
          <span class="text-base">Phút</span>
        </div>
        <div>
          <span class="block font-bold">{{ seconds }}</span>
          <span class="text-base">Giây</span>
        </div>
      </div>
    </div>
  </section>
